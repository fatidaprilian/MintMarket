...
<div class="min-h-screen bg-gray-50 py-8">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-t-xl shadow-lg overflow-hidden">
            <div class="px-6 py-12 text-center bg-gradient-to-br from-primary-50 to-primary-100">
                ...
                <p class="text-lg text-gray-700 max-w-2xl mx-auto">
                    Isi detail produk Anda untuk mulai menjual di MintMarket. Pastikan informasi akurat dan menarik!
                </p>
            </div>
        </div>
        ...
        <!-- Main Form -->
        <form action="{{ route('store.products.store') }}"
              method="POST"
              ...
              x-data="{
                  ...
                  removeImage(index) {
                      ...
                      // You might need to handle actual file input removal if it's a single file input
                      // For multiple file inputs, this simply removes the preview.
                  }
              }">
                ...
                <div class="grid grid-cols-1 gap-6">
                    ...
                    <div>
                        ...
                        <textarea id="description"
                                  ...
                                  rows="6"
                                  class="block w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm focus:ring-primary-500 focus:border-primary-500 transition duration-150"
                                  placeholder="Jelaskan detail produk Anda, fitur, bahan, ukuran, dll.">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="grid grid-cols-1 gap-6">
                    <div>
                        <label for="image-upload" class="block text-sm font-medium text-gray-700 mb-2">
                            Unggah Gambar (Maks 5 gambar, max 2MB per gambar)
                        </label>
                            ...
                            <div class="mt-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                                <template x-for="(preview, index) in imagePreviews" :key="index">
                                    <div class="relative w-full h-32 rounded-lg overflow-hidden border border-gray-300">
                                        <img :src="preview" alt="Image preview" class="w-full h-full object-cover">
                                        <button type="button" @click="removeImage(index)" class="absolute top-1 right-1 bg-red-500 text-white rounded-full p-1 text-xs">
            ...
            <div class="p-8 border-b border-gray-200">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {{-- Status Produk - Dihilangkan dari input yang dapat diedit --}}
                    {{-- Tombol status di halaman buat produk diatur otomatis ke "menunggu_verifikasi" oleh controller --}}
                    <input type="hidden" name="status" value="menunggu_verifikasi">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">
                            Status Produk
                        </label>
                        <p class="block w-full px-4 py-3 bg-gray-100 border border-gray-300 rounded-lg shadow-sm cursor-not-allowed text-gray-600">
                    ...
                    <div>
                        ...
                        {{-- Dropdown untuk memilih sesi flash sale --}}
                        <select id="flash_sale_end_date"
                                name="flash_sale_end_date"
                                class="block w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm focus:ring-primary-500 focus:border-primary-500">
                            @foreach($flashSaleSessions as $value => $label)
                                <option value="{{ $value }}" {{ (old('flash_sale_end_date') == $value) ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
            ...
            <!-- Submit Button -->
            <div class="p-8 flex justify-between"> {{-- Added flex justify-between --}}
